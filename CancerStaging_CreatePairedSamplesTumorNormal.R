...
{  
  ...
  for (gene in log2change_tumor_control$gene)
  {
	...
	gene_expression<-normalized_table[gene,]
	
	# Take the expression of genes above expression threshold for Stage_i_samples
	expr_unpaired_tumor_samples<-gene_expression[which(gene_expression[,unpaired_tumor_samples]>list_threshold_filters[[normalized_table_names]])]
	  		
	# Take the expression of genes above expression threshold for Stage_i_samples
	expr_unpaired_contol_samples<-gene_expression[which(gene_expression[,unpaired_control_samples]>list_threshold_filters[[normalized_table_names]])]
	
	# Take the expression of genes above expression threshold for Stage_i_samples
	expr_paired_tumor_samples<-gene_expression[which(gene_expression[,paired_tumor_samples]>list_threshold_filters[[normalized_table_names]])]
	
	# Take the expression of genes above expression threshold for Stage_i_samples
	expr_paired_control_samples<-gene_expression[which(gene_expression[,paired_normal_samples]>list_threshold_filters[[normalized_table_names]])]
    
	# Take p-value
	log2change_tumor_control[gene,"Pvalue"]<-t.test(x=as.vector(expr_unpaired_tumor_samples), y=as.vector(expr_unpaired_control_samples), paired = FALSE, alternative = "two.sided")$p.value	
	log2change_tumor_control_paired[gene,"Pvalue"]<-t.test(x=as.vector(expr_paired_tumor_samples), y=as.vector(expr_paired_control_samples), paired = TRUE, alternative = "two.sided")$p.value	
  }
  # FRD 
